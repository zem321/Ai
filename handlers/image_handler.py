...
def extract_error_message(resp_data: dict) -> str:
...
async def call_edit(image_bytes: bytes, prompt: str) -> bytes:
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(EDIT_URL, data=form, headers=headers, timeout=aiohttp.ClientTimeout(total=120)) as resp:
            text = await resp.text()
...
Пример: <code>сделай фон ночным городом</code>", parse_mode="HTML", reply_markup=cancel_keyboard())
    ...
    try:
        ...
        image_bytes = compress_image(downloaded.read())
