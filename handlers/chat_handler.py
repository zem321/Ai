...
SYSTEM_PROMPT = (
    ...
    "Строго запрещено: вступления 'Вот ваш файл', 'Конечно', пояснения до/после, "
    "вопросы 'нужно ли что-то еще?', 'готово?' и т.п.\n"
...
FILE_SEND_KEYWORDS = [
    ...
    "в виде файла", "файлом",
    " скинь ", " скачай", " сохрани", " txt", ".txt", " файл"
]
...
# Режим ответа при запросе файлом: "both" = текст в чат + файл, "file_only" = только файл
FILE_RESPONSE_MODE = os.getenv("FILE_RESPONSE_MODE", "both")
...
logger.info("FREEMODEL_CLAUDE_BASE = %s", FREEMODEL_CLAUDE_BASE)
...
if not FREEMODEL_API_KEY:
    logger.warning("FREEMODEL_API_KEY не задан.")
if not FREEMODEL_CLAUDE_API_KEY and not FREEMODEL_API_KEY:
    logger.warning("Ни FREEMODEL_CLAUDE_API_KEY, ни FREEMODEL_API_KEY не заданы.")
if not ASHIBALT_API_KEY:
    logger.warning("ASHIBALT_API_KEY не задан.")
...
async def call_nvidia(model_id: str, messages: list) -> tuple[str, dict]:
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(NVIDIA_CHAT_URL, json=payload, headers=headers, timeout=aiohttp.ClientTimeout(total=60)) as resp:
            ...
            debug = {"url": NVIDIA_CHAT_URL, "sent_model": model_id, "provider_model": data.get("model")}
            try:
...
async def call_freemodel_openai(raw_model: str, messages: list, base_url: str, api_key: str | None = None, extra_headers: dict | None = None) -> tuple[str, dict]:
    key = api_key or FREEMODEL_API_KEY
    if not key:
        raise Exception("API ключ не задан (FREEMODEL_API_KEY или FREEMODEL_CLAUDE_API_KEY).")
    headers = {"Authorization": f"Bearer {key}", "Content-Type": "application/json", "User-Agent": "Mozilla/5.0 (compatible; freemodel-bot/1.0)"}
    if extra_headers:
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(url, json=payload, headers=headers, timeout=aiohttp.ClientTimeout(total=60)) as resp:
            ...
            try:
                ...
                raise Exception(f"FreeModel вернул неожиданный ответ (HTTP {resp.status}, url={url}): {text[:500]}")
            if resp.status != 200:
                raise Exception(extract_api_error(data))
            logger.info("call_freemodel_openai <- responded model=%s", data.get("model"))
            debug = {"url": url, "sent_model": raw_model, "provider_model": data.get("model")}
            try:
                return data["choices"][0]["message"]["content"], debug
            except Exception:
...
async def call_ashibalt(model_id: str, messages: list) -> tuple[str, dict]:
    ...
    headers = {"Authorization": f"Bearer {ASHIBALT_API_KEY}", "Content-Type": "application/json"}
    url = f"{ASHIBALT_API_BASE.rstrip('/')}/chat/completions"
    async def request_model(raw_model: str) -> tuple[str, dict]:
        payload = {"model": raw_model, "messages": messages, "max_tokens": 2048}
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, json=payload, headers=headers, timeout=aiohttp.ClientTimeout(total=60)) as resp:
                ...
                try:
                    ...
                    raise Exception(f"Ashibalt вернул неожиданный ответ (HTTP {resp.status}, url={url}): {text[:500]}")
                if resp.status != 200:
                    raise Exception(extract_api_error(data))
    ...
    for index, raw_model in enumerate(candidate_models):
        try:
            ...
            model_not_found = ("not available" in lower_error or "not found" in lower_error or "unknown model" in lower_error)
            if model_not_found and index < len(candidate_models) - 1:
                logger.warning("call_ashibalt: model=%s недоступна, пробую алиас %s", raw_model, candidate_models[index + 1])
                continue
            raise
    raise Exception(f"Ashibalt не принял model id. Пробовал: {', '.join(candidate_models)}. Последняя ошибка: {last_error}")
...
COUNCIL_JUDGE_SYSTEM_PROMPT = (
    "Ты выступаешь в роли судьи в \"Совете ИИ-моделей\". "
    "Выбери лучший ответ или сделай синтез. "
    "Отвечай на языке исходного вопроса.\n\n"
...
async def call_council_member(model_id: str, messages: list) -> tuple[str, dict]:
    try:
        ...
        return (f"[Эта модель не смогла ответить: {e}]", {"requested_model": model_id, "error": str(e)})

def build_council_judge_messages(original_messages: list, labeled_answers: list) -> list:
    judge_messages = [{"role": "system", "content": COUNCIL_JUDGE_SYSTEM_PROMPT}]
    for msg in original_messages:
        if msg.get("role") == "system":
            continue
        judge_messages.append(msg)
    answers_text = "\n\n".join(f"Вариант ответа {label}:\n{content}" for label, content in labeled_answers)
    judge_messages.append({"role": "user", "content": f"Вот варианты ответов разных ИИ-моделей:\n\n{answers_text}\n\nВыбери лучший или сделай синтез, дай финальный ответ."})
...
async def call_ai_council(messages: list) -> tuple[str, dict]:
    ...
    for label, model_id, (content, member_debug) in zip(COUNCIL_LABELS, COUNCIL_MEMBER_MODELS, results):
        ...
        members_debug.append({"label": label, "model": model_id, "debug": member_debug})
    ...
    debug = {"url": "Совет ИИ-моделей (3 модели + судья)", "sent_model": judge_debug.get("sent_model"), "provider_model": judge_debug.get("provider_model"), "council_members": members_debug, "judge_debug": judge_debug}
...
async def call_ai(model_id: str, messages: list) -> tuple[str, dict]:
    ...
    if model_id == COUNCIL_MODEL_ID:
        ...
        if raw_model.startswith("claude-"):
            ...
            openrouter_headers = {"HTTP-Referer": "https://t.me/", "X-Title": "Telegram AI Bot"}
            logger.info("call_ai: claude маппинг %s -> %s, base=%s", raw_model, mapped_model, FREEMODEL_CLAUDE_BASE)
            content, debug = await call_freemodel_openai(mapped_model, messages, FREEMODEL_CLAUDE_BASE, api_key=claude_key, extra_headers=openrouter_headers)
        else:
...
async def handle_text(message: Message, state: FSMContext):
    ...
    # детект запроса файлом
    want_file = any(kw in low for kw in FILE_SEND_KEYWORDS)
    is_resend_command = low.strip() in FILE_RESEND_COMMANDS
    file_request_only = want_file and is_resend_command

    # Команда "перешли последний ответ файлом"
    ...
    # Для файловой генерации - добавляем жёсткую инструкцию
    user_content_for_model = text
    if want_file:
        user_content_for_model = (
            text +
            "\n\n[СИСТЕМНО: ВЫДАЙ ТОЛЬКО ЧИСТОЕ СОДЕРЖИМОЕ ФАЙЛА. "
            "Без вступлений, пояснений, вопросов. Только контент.]"
        )
...
async def handle_photo(message: Message, state: FSMContext):
    ...
    try:
        ...
        await edit_error(status_msg, "Ошибка при обработке фото. Проверьте, что выбрана модель с поддержкой Vision", e)
...
async def handle_document(message: Message, state: FSMContext):
    ...
    if mime_type.startswith("image/"):
        ...
        try:
            ...
            await edit_error(status_msg, "Ошибка при обработке изображения. Проверьте, что выбрана модель с поддержкой Vision", e)
        return

    # Текстовый / PDF / DOCX файл
    if document.file_size and document.file_size > MAX_FILE_BYTES:
        await message.answer(f"Файл слишком большой ({document.file_size / 1024 / 1024:.1f} МБ). Максимум {MAX_FILE_BYTES // 1024 // 1024} МБ.", reply_markup=cancel_keyboard())
        return
...
async def handle_unsupported_message(message: Message):
    await message.answer("Я могу обработать текст, фото, изображение-файл, а также документы: TXT, MD, JSON, CSV, код, PDF, DOCX.\nПросто пришлите файл с подписью-заданием, если нужно.", reply_markup=cancel_keyboard())
