...
# Модели ChatGPT (помним историю, поддержка чата + фото)
CHATGPT_MODELS = {
...
# Модели, поддерживающие фото (для информации в коде)
VISION_MODELS = {
    "freemodel/gpt-5.4-nano",
    "freemodel/gpt-5.5",
...
def model_select_keyboard(current: str = "") -> InlineKeyboardMarkup:
    """
    Клавиатура выбора модели с двумя кнопками: ChatGPT / Other
    Под ChatGPT только chatgpt модели
    Под Other — все остальные текстовые модели + Vision
    """
    buttons = []
    ...
    # Other группа (текстовые + Vision, которые не в ChatGPT)
